<?php
class LastEventsWidgetBig extends WP_Widget {
	/**
	 * Init the class and parse the widget title to Wordpress Widget Page
	 */
    function LastEventsWidgetBig() {
        parent::WP_Widget(false, $name = 'PPSN: Letzte Events Big', $width = 500, $widget_options = array("description" => "Zeigt die letzten Events an."));
    }
    ...
    function getLastEvents ($html = "") {
    	global $wpdb;
    	$events = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."posts WHERE `post_type` = 'event' AND `post_date` >= NOW() ORDER BY `post_date` ASC LIMIT 0,11");
    	foreach ($events as $event) {
    		$date = explode(" ",$event->post_date);
    		$date = explode("-",$date[0]);
    		$date = $date[2].".".$date[1].".".$date[0];
    		$html .= '<dl>
				       <dt>Am '.$date.' in '.$event->post_name.'</dt>
				       <dd>'.$event->post_title.' ... <a href="'.$event->to_ping.'">mehr erfahren</a></dd>
				      </dl>';
    	}
		return $html;
    }
